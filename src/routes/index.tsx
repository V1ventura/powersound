...
import radioBluetooth from "@/assets/Instalação de RadioBluetooth e fuel teh.jpeg";
import autofalante from "@/assets/Instalação de autofalante.jpeg";
import lavagem from "@/assets/Lavagem detalhada.jpeg";
import suporte from "@/assets/Suportequebrasol com auto falante.jpg";
import trocaFarol from "@/assets/Trocadefaról para Ultra led.jpg";
import vitrificacao from "@/assets/Vitrificação.png";
    ...
    <main className="overflow-hidden bg-background text-foreground">
      ...
      <section id="servicos" className="section-shell carbon-bg">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          <div className="reveal flex flex-col justify-between gap-6 md:flex-row md:items-end"><SectionTitle eyebrow="EXPERTISE POWERSOUND">CUIDADO EM CADA <span>DETALHE.</span></SectionTitle><p className="max-w-md text-muted-foreground">Agora com trabalhos reais da PowerSound para mostrar, além do serviço, o nível de acabamento entregue.</p></div>
          <div className="mt-12 grid gap-px overflow-hidden border border-border bg-border sm:grid-cols-2 lg:grid-cols-5">
            {services.map((service, i) => <article key={service.title} className="service-card reveal">
              {service.image && <div className="service-card-image"><img src={service.image} alt={service.title} loading="lazy" /></div>}
              <div className="mb-6 flex items-start justify-between"><span className="font-display text-xs text-primary">{String(i + 1).padStart(2, "0")}</span>{i % 3 === 0 ? <Sparkles /> : i % 3 === 1 ? <ShieldCheck /> : <Wrench />}</div>
              <h3>{service.title}</h3><p>{service.desc}</p><a href={WHATSAPP} target="_blank" rel="noreferrer">SAIBA MAIS <ArrowRight /></a>
            </article>)}
          </div>
        </div>
      </section>
      ...
      <section className="section-shell bg-surface">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          <div className="reveal grid gap-10 lg:grid-cols-[0.7fr_1.3fr] lg:items-end"><SectionTitle eyebrow="RESULTADO REAL">ANTES, DEPOIS E <span>RESULTADO.</span></SectionTitle><p className="max-w-lg text-muted-foreground lg:justify-self-end">Um exemplo real de recuperação de farol, usando as imagens enviadas para mostrar a evolução do serviço.</p></div>
          <div className="mt-12 grid gap-3 md:grid-cols-3">
            {[[farolAntes, "ANTES", "Condição inicial"], [farolDepois, "DEPOIS", "Após a recuperação"], [farolResultado, "RESULTADO", "Acabamento final"]].map(([src, label, detail]) => <figure key={label as string} className="real-result reveal"><div><img src={src as string} alt={`${label} - recuperação de farol`} loading="lazy" /></div><figcaption><strong>{label as string}</strong><span>{detail as string}</span></figcaption></figure>)}
          </div>
          <div className="compare reveal mt-8" style={{ "--compare": `${compare}%` } as React.CSSProperties}><img src={farolAntes} alt="Farol antes da recuperação" className="compare-img compare-before"/><div className="compare-after"><img src={farolDepois} alt="Farol depois da recuperação" className="compare-img after-treatment"/></div><div className="compare-line"><div><ChevronLeft/><ChevronRight/></div></div><span className="compare-label left">ANTES</span><span className="compare-label right">DEPOIS</span><input aria-label="Comparar recuperação do farol" type="range" min="5" max="95" value={compare} onChange={(e) => setCompare(Number(e.target.value))}/></div>
        </div>
      </section>
